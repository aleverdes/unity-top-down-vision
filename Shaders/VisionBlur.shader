Shader "GrozaGames/Vision/Blur Effect"
{
    HLSLINCLUDE
        ...
        float4 BlurVertical (Varyings input) : SV_Target
        {
            ...
            float3 color = 0;
            float blurPixels = _VerticalBlur * min(_ScreenParams.x, _ScreenParams.y);
        ...
        float4 BlurHorizontal (Varyings input) : SV_Target
        {
            ...
            float3 color = 0;
            float blurPixels = _HorizontalBlur * min(_ScreenParams.x, _ScreenParams.y);
            for(float i = -BLUR_SAMPLES_RANGE; i <= BLUR_SAMPLES_RANGE; i++)
            {
                float2 sampleOffset =
